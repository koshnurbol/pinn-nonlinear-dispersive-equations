...
def kawahara_residual_pinn(x, t):
    """
    Standard PINN residual for Kawahara:
...
if USE_LBFGS:
    print("\nStarting L-BFGS refinement for Kawahara...")
...
def evaluate_pde_loss_fixed(N_eval=2000):
    ...
    Here u_xxxxx is again computed by AD.
    """
    model.eval()
...
with torch.no_grad():
    ...
    for tv in t_grid:
        ...
        u_exact = exact_kawahara(x_diag, t_col)
...
plt.title(f"Kawahara ablation: {RUN_NAME}")
plt.yscale("log")
...
plt.title(f"Kawahara energy drift: {RUN_NAME}")
plt.yscale("log")
plt.grid(True, which="both", alpha=0.3)
plt.tight_layout()
...
plt.title(f"Kawahara PINN ablation: {RUN_NAME}, L2={final_l2:.4e}")
plt.xlabel("x")
plt.ylabel("u(x,t)")
plt.legend()
plt.grid(True)

...
RUN_NAME = f"kdv_norm_{USE_INPUT_NORMALIZATION}_energy_{USE_ENERGY_LOSS}_lbfgs_{USE_LBFGS}_seed_{SEED}"
...
def exact_soliton(x, t, v=1.0, x0=-5.0):
    """
    Exact one-soliton solution for:

        u_t + 6 u u_x + u_xxx = 0

    u(x,t) = v/2 sech^2( sqrt(v)/2 * (x - v t - x0) )
    """
    v_tensor = torch.tensor(v, dtype=x.dtype, device=x.device)
    arg = (torch.sqrt(v_tensor) / 2.0) * (x - v * t - x0)
    return (v / 2.0) * (1.0 / torch.cosh(arg)) ** 2
...
model = KdV_PINN(width=128, depth=4).to(device)
...
lambda_ic = 10.0
lambda_bc = 1.0
lambda_energy = 0.1 if USE_ENERGY_LOSS else 0.0
...
x_energy = torch.linspace(-L, L, 300).view(-1, 1).to(device)
dx = x_energy[1] - x_energy[0]

with torch.no_grad():
    u_initial_exact = exact_soliton(x_energy, torch.zeros_like(x_energy))
    E0 = torch.sum(u_initial_exact ** 2) * dx


def compute_energy_pinn(t_scalar):
    """
    E(t) = integral u(x,t)^2 dx
    """
...
for epoch in range(adam_epochs):
    ...
    residual = kdv_residual_pinn(x_col, t_col)
    ...
    u_ic_exact = exact_soliton(x_ic, t_ic)
...
if USE_LBFGS:
    ...
    def closure():
        ...
        residual = kdv_residual_pinn(x_col_f, t_col_f)
        loss_pde = torch.mean(residual ** 2)

        u_ic_pred = model(x_ic_f, t_ic_f)
        u_ic_exact = exact_soliton(x_ic_f, t_ic_f)
...
def evaluate_final_time_l2():
    ...
    with torch.no_grad():
        x_test = torch.linspace(-L, L, 500).view(-1, 1).to(device)
        t_test = torch.full_like(x_test, T_max)

        u_pred = model(x_test, t_test)
        u_exact = exact_soliton(x_test, t_test)
...
def evaluate_full_and_interior_l2():
    ...
    with torch.no_grad():
        ...
        u_exact = exact_soliton(x_diag, t_fin)
...
def evaluate_space_time_l2(Nx=500, Nt=80):
    model.eval()
    ...
    with torch.no_grad():
        ...
        for tv in t_grid:
            ...
            u_exact = exact_soliton(x_grid, t_col)
...
def evaluate_energy_drift():
    ...
    with torch.no_grad():
        t_energy_diag = torch.linspace(0.0, T_max, 60)
        energy_values = []
...
def evaluate_pde_loss_fixed(N_eval=3000):
    ...
    residual = kdv_residual_pinn(x_eval, t_eval)
...
space_time_l2 = evaluate_space_time_l2(Nx=500, Nt=80)
...
with torch.no_grad():
    ...
    t_grid = torch.linspace(0.0, T_max, 60)
    ...
    for tv in t_grid:
        ...
        u_exact = exact_soliton(x_diag, t_col)
...
plt.title(f"KdV ablation: {RUN_NAME}")
plt.yscale("log")
...
plt.title(f"KdV energy drift: {RUN_NAME}")
plt.yscale("log")
plt.grid(True, which="both", alpha=0.3)
plt.tight_layout()
...
with torch.no_grad():
    for i, t_val in enumerate(times_to_plot):
        ...
        u_exact_plot = exact_soliton(x_test, t_plot)
...
plt.title(f"KdV PINN ablation: {RUN_NAME}, L2={final_l2:.4e}")
plt.xlabel("x")
plt.ylabel("u(x,t)")
plt.legend()
plt.grid(True)
